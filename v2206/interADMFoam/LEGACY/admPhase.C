...
namespace Foam
{
    defineTypeNameAndDebug(admPhase, 0);
}
 
 
// * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * * //
 
void Foam::admPhase::calcNu()
{
    nuModel1_->correct();
    nuModel2_->correct();
...
:
    ...
    phase1Name_(this->get<wordList>("phases")[0]),
    phase2Name_(this->get<wordList>("phases")[1]),
...
Foam::admPhase::mu(const label patchI) const
{
...
Foam::admPhase::nuf() const
{
    ...
    (
        ...
        (
            alpha1f*rho1_*fvc::interpolate(nuModel1_->nu())
          + alpha2f*rho2_*fvc::interpolate(nuModel2_->nu())
        )/(alpha1f*rho1_ + alpha2f*rho2_)
    );
}
 
 
bool Foam::admPhase::read()
{
    if (regIOobject::read())
    {
        if
